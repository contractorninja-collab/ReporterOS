...
test('exports list, completion, and per-lane markdown timestamps', () => {
  const csv = buildMarkdownListCSV({
    ...
    items: [{
      skuCode: 'SKU-1', productName: 'Test product', brand: 'Brand', category: 'Footwear',
  ...
  assert.match(header, /^List Created At \(UTC\),List Completed At \(UTC\),SKU/)
  assert.match(header, /Ring Mall Tagged At \(UTC\)/)
  assert.match(row, /^2026-06-19T09:28:50\.499Z,2026-06-19T09:29:11\.578Z,SKU-1/)
